...
test_that("Pvalues displayed are correct.", {
  ...
  expect_equal(tab_quanti(data = mtcars, x = mpg, y = am, prec = 1, test = "wilc", chif_pval = 4)[1, 6] %>% as.numeric(),
               round(wilcox.test(mtcars$mpg ~ mtcars$am)$p.value, 4))
})

# Noms de colonnes pas de la bonne longueur
test_that("Checking the length of nomcol specified.", {
  expect_error(tab_quanti(mtcars, mpg, nomcol = c("Test", "of", "nomcol's", "length")))
  expect_error(tab_quanti(mtcars, mpg, am, nomcol = c("Test", "of", "nomcol's", "length")))
})

# Checking for the crossing variable
test_that("Checking for the crossing variable.", {
  mtcars$am <- 1
  expect_error(tab_quanti(mtcars, mpg, am))
  expect_message(tab_quanti(mtcars, mpg, cyl))
})

# Il faut donner une variable numérique
test_that("Checking for the variable to describe.", {
  mtcars$mpg <- as.character(mtcars$mpg)
  expect_error(tab_quanti(mtcars, mpg))
  mtcars$disp <- as.factor(mtcars$disp)
  expect_error(tab_quanti(mtcars, disp))
})
