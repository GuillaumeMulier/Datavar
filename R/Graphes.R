...
#' @param Poids Vector of weights
#'
#' @return Single bar plot.
#'
GGBar <- function(VarBinaire, VarCroise = NULL, Poids, Prec) {
  # The percent in annotation is quite unadapted and I didn't find a way to add it satisfactorily
  if (is.null(VarCroise)) {
    .Data <- data.frame(varbin = VarBinaire, poids = Poids)
    Graphe <- ggplot2::ggplot(.Data, ggplot2::aes(x = 1, fill = factor(varbin), weights = poids)) +
    ...
    .Data <- data.frame(varbin = VarBinaire, varcrois = as.numeric(VarCroise == unique(VarCroise[1])), poids = Poids)
    Graphe <- ggplot2::ggplot(.Data, ggplot2::aes(x = varcrois, fill = factor(varbin), weights = poids)) +
...
#'
GGHist <- function(VarQuanti, Poids, Bins = 10) {
    Graphe <- ggplot2::ggplot(data = NULL, ggplot2::aes(x = VarQuanti, weights = Poids)) +
